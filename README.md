...
> **1.2.1**：在 1.1.0「让方块能显示」的基础上，修复石碑在基岩版**破坏碎屑显示为紫色缺失贴图**的问题（并很可能一并修复"部分石碑放下去秒消失"——见 [§3.5](#35-方块转换修复)）。物品栏缩略图维持上半截显示（按需求不改动）。
...
> 关于问题 ②（部分石碑放下去秒消失）：在去掉跨包贴图引用、让默认材质可解析后**很可能一并解决**（理由如上）。若实测仍未好，则更可能是 Geyser 对**两格高自定义方块**的放置预测/状态同步的固有问题（Geyser 官方文档亦提示自定义方块组件不够稳定），需另行排查。

## 4. 安装使用

1. 把 `hydraulic-leaffix-1.2.1.jar` 放进服务端的 `mods/` 文件夹。
...
```
脚本会：从服务端 `libraries/` 取 sponge-mixin 与 ASM、从 `.fabric/` 取 MixinExtras、从 `mods/bedframe-*.jar` 解出与运行时一致的 `pack-converter` 类、从 `mods/hydraulic-fabric*.jar` 取 Hydraulic 类（供 `@WrapOperation` 处理方法声明真实的 `Materials$Material` 形参类型）作为编译依赖，编译全部 Mixin 并打包出 `hydraulic-leaffix-1.2.1.jar`。
...
├── README.md
├── build.sh                                  # 复现编译脚本
├── hydraulic-leaffix-1.2.1.jar               # 编译好的成品
└── src/main/
...
- **1.2.1**：`BlockPackModuleMixin` 改为 `@WrapOperation` 包裹 `Materials$Material.textures()`，把 `minecraft:` 的 `#particle` 值改写为方块自己的**同包面贴图**，使基岩默认材质实例 `"*"`（破坏碎屑来源）指向确定存在的贴图 → 碎屑不再变紫（很可能一并修复"部分石碑放下去秒消失"）。已实测：Mixin 正常命中、启动无异常、转换日志出现 `Redirected break-particle texture ...`。
...
- 若"部分石碑放下去秒消失"在本版本后仍存在，则更可能是 Geyser 对**两格高自定义方块**放置同步的固有问题（与本模组可修复范围无关），需结合基岩端实测进一步排查。
